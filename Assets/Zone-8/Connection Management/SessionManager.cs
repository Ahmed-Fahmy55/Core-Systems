...
namespace Zone8.Multiplayer.ConnectionManagement
{
    ...
    public class SessionManager<T> where T : struct, ISessionPlayerData
    {
            ...
            if (_hasSessionStarted)
            {
                ...
                if (_clientIDToPlayerId.TryGetValue(clientId, out var playerId))
                {
                    ...
                    if (playerData != null && playerData.Value.ClientID == clientId)
                    {
                        ...
                        Debug.Log("mark client disconnected: ID -> " + playerId);
                    }
                }
            }
            else
        ...
        public void SetupConnectingPlayerSessionData(ulong clientId, string playerId, T sessionPlayerData)
        {
            ...
            if (isReconnecting)
            {
                Debug.Log("Player reconnected: ID ->" + playerId);
        ...
        public List<ulong> GetDisconnectedClients()
        {
            List<ulong> disconnectedClients = new();
            foreach (var data in _clientData)
            {
                if (!data.Value.IsConnected) disconnectedClients.Add(data.Value.ClientID);
            }
