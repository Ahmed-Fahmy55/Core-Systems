...
namespace ASoliman.Utils.ClipboardPlus
{
    ...
    public static class ClipboardContextMenu
    {
        ...
        [MenuItem("CONTEXT/Component/Clipboard Plus/Auto Save", true, 100)]
        private static bool ValidateAutoSave(MenuCommand command)
        {
            return EditorApplication.isPlaying && PlayModePersistenceManager.Instance.IsRecording;
        }

        [MenuItem("CONTEXT/Component/Clipboard Plus/Auto Save")]
        private static void AutoSave(MenuCommand command)
        {
            var component = command.context as Component;
            if (component != null)
            {
        ...
        [MenuItem("CONTEXT/Component/Clipboard Plus/Save Snapshot", true, 101)]
        private static bool ValidateSaveSnapshot(MenuCommand command)
        {
            return EditorApplication.isPlaying && PlayModePersistenceManager.Instance.IsRecording;
        }

        [MenuItem("CONTEXT/Component/Clipboard Plus/Save Snapshot")]
        private static void SaveSnapshot(MenuCommand command)
        {
            var component = command.context as Component;
            if (component != null)
            {
